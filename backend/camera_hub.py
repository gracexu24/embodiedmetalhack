"""Owns camera3 (the reference/model-house scan camera).
...
camera3 is a separate, dedicated camera pointed at a human-built reference house.
"""
...
class CameraUnavailableError(RuntimeError):
    """Raised when camera3 could not be opened or has not produced a frame yet."""
...
class CameraHub:
    ...
    def start(self) -> None:
        if not self._config:
            log.warning("No camera3 entry in config.yaml; reference-scan camera disabled.")
            return
        try:
            ...
            if not capture.isOpened():
                capture.release()
                raise RuntimeError(
                    f"Could not open camera3 at index {self._config['index']}."
                )
        except Exception:
            log.warning(
                "camera3 unavailable; the reference-scan panel will report camera-unavailable.",
                exc_info=True,
            )
            return
    ...
    def latest_frame(self) -> np.ndarray:
        ...
        with self._lock:
            if self._latest_frame is None:
                raise CameraUnavailableError("camera3 has not produced a frame yet.")

...
def _port_has_motor(port: str) -> bool:
    """True if any SO-100 servo answers a ping on this port.

    Distinguishes an arm adapter from other ``/dev/cu.usbmodem*`` gadgets — USB-C
    docks, display/capture dongles — that share the name pattern but have no motors
    (seen on real hardware: a GsCooLink "USB BillBoard" enumerating as usbmodem).
    """
    handler = scs.PortHandler(port)
    packet = scs.PacketHandler(PROTOCOL_END)
    try:
        if not handler.openPort() or not handler.setBaudRate(BAUD_RATE):
            return False
        # any() short-circuits, so a real arm (motor 1 replies) costs one ping; only
        # a motorless port pays the full timeout across all six ids.
        return any(packet.ping(handler, motor_id)[1] == scs.COMM_SUCCESS for motor_id in (1, 2, 3, 4, 5, 6))
    except OSError:  # port vanished mid-probe
        return False
    finally:
...
def detect_arm_ports() -> tuple[str, ...]:
    """Every SO-100 arm currently on the bus (macOS device names).

    Globs the usbmodem devices, then keeps only ports where a servo actually answers
    — other usbmodem gadgets (docks, dongles) match the glob but no motor replies. If
    nothing answers at all (e.g. the arms are powered off), fall back to the raw glob
    so callers' bus errors still name real ports instead of reporting none found.
    """
    candidates = tuple(sorted(glob.glob("/dev/cu.usbmodem*")))
    arms = tuple(port for port in candidates if _port_has_motor(port))
    return arms or candidates
